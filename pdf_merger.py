...
check if a non pdf file is dropped 
what happens if only 1 or no pdf is provided #
no empty output name allowed (and no one without pdf ending?) 
add entry to give ouput path (if empty use working dir)
use listbox.get instead of the list creating stuff #
exract file name from path #
check if pdf #
add alert for success?
scrollable machen
what happesn if nothing is submitted
add automatic name
only show name not path in listbox?
error handling
deselect an item?
builtin converter?
add button to allow that converted files stay stored ?

# UNDERSTAND DIFFERENCE BETWEEN GETCWD AND OS DIRNAME(__file__)
"""
